  ...
  return (
    <Box className="my-scheduled-interviews">
      ...
      <Box className="filters-container" mb={3}>
        <Typography variant="subtitle2" sx={{ fontWeight: '600', marginBottom: '16px', color: isDarkMode ? '#64b5f6' : '#4a4540' }}>
          סנן ומיין
        </Typography>
        
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <FilterListIcon sx={{ fontSize: '24px', color: isDarkMode ? '#64b5f6' : '#4a4540' }} />
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <SortIcon sx={{ fontSize: '24px', color: isDarkMode ? '#64b5f6' : '#4a4540' }} />
            <FormControl sx={{ minWidth: 180 }} size="small">
      ...
      {/* Results Summary */}
      {interviews.length > 0 && (
        <Typography variant="body2" sx={{ color: isDarkMode ? '#b0b0b0' : '#666', marginBottom: '20px', fontWeight: '500' }}>
      ...
      {interviews.length === 0 ? (
        <Box className="interviews-empty">
          <Box sx={{ textAlign: 'center' }}>
            <EventIcon sx={{ fontSize: '48px', color: isDarkMode ? '#64b5f6' : '#a89f9a', marginBottom: '16px' }} />
            <Typography variant="h6" sx={{ color: isDarkMode ? '#b0b0b0' : '#666', marginBottom: '8px' }}>
              אין ריאיונות מתוכננים כרגע
            </Typography>
            <Typography variant="body2" sx={{ color: isDarkMode ? '#999' : '#999' }}>
        ...
        <Box className="interviews-empty">
          <Box sx={{ textAlign: 'center' }}>
            <FilterListIcon sx={{ fontSize: '48px', color: isDarkMode ? '#64b5f6' : '#a89f9a', marginBottom: '16px' }} />
            <Typography variant="h6" sx={{ color: isDarkMode ? '#b0b0b0' : '#666', marginBottom: '8px' }}>
              לא נמצאו ריאיונות
            </Typography>
            <Typography variant="body2" sx={{ color: isDarkMode ? '#999' : '#999' }}>
                ...
                <Card className="interview-card" sx={{ width: '100%' }}>
                  <CardContent>
                    ...
                    <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={2}>
                      <Box>
                        <Typography variant="h6" component="h3" sx={{ fontWeight: '600', color: isDarkMode ? '#64b5f6' : '#4a4540' }}>
                          {interviewTypeLabel}
                        </Typography>
                        <Chip
                    ...
                    <Box className="interview-meta">
                      ...
                      <Box display="flex" alignItems="center" gap={1.5}>
                        <EventIcon sx={{ fontSize: '20px', color: isDarkMode ? '#64b5f6' : '#4a4540' }} />
                        <Typography variant="body2" sx={{ color: isDarkMode ? '#b0b0b0' : '#444', fontWeight: '500' }}>
                          {startDateTime.date}
                        </Typography>
                      </Box>
                      ...
                      {/* Time */}
                      <Box display="flex" alignItems="center" gap={1.5}>
                        <AccessTimeIcon sx={{ fontSize: '20px', color: isDarkMode ? '#64b5f6' : '#4a4540' }} />
                        <Typography variant="body2" sx={{ color: isDarkMode ? '#b0b0b0' : '#444', fontWeight: '500' }}>
                      ...
                      {interview.place && (
                        <Box display="flex" alignItems="center" gap={1.5}>
                          <LocationOnIcon sx={{ fontSize: '20px', color: isDarkMode ? '#64b5f6' : '#4a4540' }} />
                          <Typography variant="body2" sx={{ color: isDarkMode ? '#b0b0b0' : '#444', fontWeight: '500' }}>
                            {interview.place}
                          </Typography>
                        </Box>
                      ...
                      {interview.comments && (
                        <Box display="flex" alignItems="flex-start" gap={1.5}>
                          <ChatIcon sx={{ fontSize: '20px', color: isDarkMode ? '#64b5f6' : '#4a4540', marginTop: '2px' }} />
                          <Typography variant="body2" sx={{ color: isDarkMode ? '#999' : '#666', fontStyle: 'italic', lineHeight: '1.5' }}>
                            {interview.comments}
                          </Typography>
                        </Box>
                      )}
